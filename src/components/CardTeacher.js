...
class CardTeacher extends React.Component {
    ...
    renderTeacher() {
        ...
        let { city, user, school, subjects, job_position, avatar, latitude, longitude } = data;
        ...
        const dataSubject = _.isEmpty(subjects) ? [] : subjects;
        let a = "";
        a += dataSubject.map((item, index) => ` ${item.name}`);
        return (
            ...
            if (Platform.OS == "ios") {
                ...
                return (
                    <View>
                        ...
                        <AndTouchableOpacity {...this.props} onPress={() => onPress(data)}>
                            {this.renderTeacher()}
                            <View
                                style={{
                                    marginLeft: PD.PADDING_4,
