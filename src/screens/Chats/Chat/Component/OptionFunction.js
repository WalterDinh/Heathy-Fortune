...
class OptionFunction extends Component {
    ...
    onCreateTemplate() {
        const { message, onAdd = () => {} } = this.props;
        onAdd(message);
    }
    async _funcCopy() {
    ...
    _funcRemoveMyBubbleMes() {
        ...
        firebase
            ...
            .once("value", childSnapshot => {
                ...
                const objDelete = chatFn.convertDeleteMyMessage(value[0]);
                console.log("sucessss", value[0]);
                value[0].text = "Bạn đã xóa một tin nhắn";
                let params = chatFn.lastMessParam(value[0],roomId, value[0].user, value[0].index).lastMessage;
                firebase
                    .database()
                    .ref(`/chat-group/${roomId}/${key[0]}`)
                    .update(objDelete)
                    .then(value => {
                        console.log("sucessss", params);
    ...
    _funcRemoveOpBubbleMes() {
        ...
        firebase
            ...
            .once("value", childSnapshot => {
                ...
                const key = Object.keys(message);
                const value = Object.values(message);
                const objDelete = chatFn.convertDeleteMessage(value[0]);
                console.log("sucessss", value[0]);
                value[0].text = "Bạn đã xóa một tin nhắn";
                let params = chatFn.lastMessParam(value[0], roomId, value[0].user, value[0].index).lastMessage;
                firebase
    ...
    _openEditBox() {
        ...
        firebase
            ...
            .once("value", childSnapshot => {
                const message = childSnapshot.toJSON();
                const key = Object.keys(message);
                const objValue = Object.values(message)[0];
    ...
    renderCopy(disabledCopy) {
        return (
            <TouchableOpacity disabled={disabledCopy} onPress={() => this._pressFunction("copy")} style={styles.btn}>
                <Icon name="md-copy" style={{ color: disabledCopy ? Colors.Deluge : Colors.Deluge }} />
    ...
    renderDelete() {
        ...
        return (
            <TouchableOpacity disabled={!mine} onPress={() => this._pressFunction("remove")} style={styles.btn}>
                <Icon name="md-trash" style={{ color: mine ? "firebrick" : Colors.Deluge }} />
    ...
    renderEdit() {
        ...
        return (
            <TouchableOpacity disabled={!mine} onPress={() => this._pressFunction("edit")} style={styles.btn}>
                <Icon name="ios-create" style={{ color: mine ? Colors.Deluge : Colors.Deluge }} />
    ...
    renderAddTemplate() {
        const { mine } = this.props;
        return (
            <TouchableOpacity disabled={!mine} onPress={() => this._pressFunction("add")} style={styles.btn}>
                <Icon name="md-add" style={{ color: mine ? Colors.Deluge : Colors.Deluge }} />
                <AppText text={I18n.t("optionFunc.template")} style={styles.txtContent} />
            </TouchableOpacity>
        );
    }
    renderForward() {
        return (
            <TouchableOpacity onPress={() => this._pressFunction("forward")} style={styles.btn}>
                <Icon name="ios-redo" style={{ color: Colors.Deluge }} />
    ...
    renderReply() {
        return (
            <TouchableOpacity onPress={() => this._pressFunction("reply")} style={styles.btn}>
                <Icon name="ios-undo" style={{ color: Colors.Deluge }} />
        ...
        return (
            <View style={{ flexDirection: "row", alignItems: "center" }}>
                <TouchableOpacity hitSlop={hitSlop} style={styles.arrowBtn} onPress={this.leftArrow}>
                    <Icon name="ios-arrow-back" style={{ color: Colors.Deluge }} />
                </TouchableOpacity>
                <ScrollView
                ...
                <TouchableOpacity hitSlop={hitSlop} style={styles.arrowBtn} onPress={this.rightArrow}>
                    <Icon name="ios-arrow-forward" style={{ color: Colors.Deluge }} />
                </TouchableOpacity>
            </View>
        );
    }
    ...
    _onEditMessage() {
        ...
        let editMessage = [editedMessage];
        let params = chatFn.lastMessParam(editMessage[0], roomId, editMessage[0].user, editMessage[0].index)
            .lastMessage;
        firebase
            .database()
            .ref(`/chat-group/${roomId}/${key[0]}`)
