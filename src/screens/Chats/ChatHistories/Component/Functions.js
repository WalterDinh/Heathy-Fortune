...
export const createGroupName = (name, users, localUserId, dataContact, type) => {
    console.log("name, users, localUserId, dataContact, type", name, users, localUserId, dataContact, type);
    ...
    if (_.isEmpty(name)) {
        ...
        for (const i in users) {
            const { first_name, last_name, id, img_url } = users[i];
            if (id !== localUserId) {
                groupName += `${first_name} ${last_name}`;
                avatar = img_url || AVATAR;
            }
            console.log("users[i].user", users[i]);
            // if (users.length - 1 == i) {
