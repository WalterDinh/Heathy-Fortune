...
class ShareContact extends React.Component {
                    ...
                    firebase
                        ...
                        .then(async response => {
                            ...
                            chatHistoriesAction.updateLastMessage(paramsLastMessage).then(() => {
                                setTimeout(() => {
                                    let params = chatFn.lastMessParam(
                                        forwardMess,
                                        chatRoomInfo.id,
                                        user,
                                        forwardMess.index
    ...
    renderItem(item, index) {
        ...
        return <CardContactForward key={index} item={item} onPress={() => this.navigateToChat(item)} />;
    }
        ...
        if (value.trim()) {
            ...
            for (let i in backupData) {
                if (!_.isEmpty(backupData[i].about_user.phone_number.toLowerCase())) {
                    if (
                        _.includes(backupData[i].nickname.toLowerCase(), value.toLowerCase()) ||
