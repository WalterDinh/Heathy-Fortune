...
class AutocompleteTag extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            translateY: new Animated.Value(DEVICE.DEVICE_HEIGHT * 2),
            value: "",
            selectedTag: [],
            tagData: []
        };
    ...
    componentDidUpdate(prevProps) {
        ...
        if (prevProps.show !== show) {
            if (this.props.show) {
                ...
                Animated.spring(this.state.translateY, {
                    toValue: DEVICE.DEVICE_HEIGHT * 2,
                    duration: 1000,
                    useNativeDriver: true
                }).start();
            }
    ...
    _onSearch(text) {
        ...
        this.setState({ value: text }, () => {
            if (text) {
                ...
                for (i in tagData.results) {
                    const stringSearch = removeDiacritics(tagData.results[i].tag_name.toLowerCase());
                    if (_.includes(stringSearch, textSearch)) {
                        dataTag.push(tagData.results[i]);
                    }
                }
            } else {
                dataTag = tagData.results;
            }
    ...
    _onClose() {
        ...
        const { tagData, data } = userReducer;
        Keyboard.dismiss();
        this.setState({ tagData: tagData.results });
        onClose();
    }
        ...
        return (
            <View style={styles.tagWrap}>
                <View style={styles.textTagWrap}>
                    <AppText numberOfLines={1} text={item.tag_name} style={styles.textTag} />
                </View>
...
AutocompleteTag = connect(mapStateToProps)(AutocompleteTag);
